...
static void usage(bool error) {
  fprintf(
      ...
      "\n"
      "Btrfs represents a range of data in a file with a \"file extent\". "
      "Each\n"
      ...
      "\"chunk\". Each chunk maps has a profile (i.e., RAID level) and maps "
      "to\n"
      "one or more physical locations, or \"stripes\", on disk. The extent may "
      "be\n"
      "\"encoded\" on disk (currently this means compressed, but in the future "
      "it\n"
      "may also be encrypted).\n"
      "\n"
      "An explanation of each printed field and its corresponding on-disk "
      "data\n"
      ...
      "\n"
      "Inline extents are stored with the metadata of the filesystem; this "
      "tool\n"
...
static int print_extents(int fd, struct chunk *chunks, size_t num_chunks) {
  ...
  puts("FILE OFFSET\tFILE SIZE\tEXTENT OFFSET\tEXTENT TYPE\tLOGICAL "
       "SIZE\tLOGICAL OFFSET\tPHYSICAL SIZE\tDEVID\tPHYSICAL OFFSET");
  ...
  for (;;) {
    ...
    if (type == BTRFS_FILE_EXTENT_INLINE) {
      ...
      extent_offset = 0;
      physical_size =
          (header->len - offsetof(struct btrfs_file_extent_item, disk_bytenr));
    ...
    if (type == BTRFS_FILE_EXTENT_INLINE || type == BTRFS_FILE_EXTENT_REG ||
        type == BTRFS_FILE_EXTENT_PREALLOC) {
      printf("%" PRIu64 "\t%" PRIu64 "\t", file_size, extent_offset);
    } else {
      printf("\t\t");
    }
    ...
    if (item->other_encoding) {
      printf(",other_encoding=%u", le16_to_cpu(item->other_encoding));
    }
    if (chunk) {
      ...
      default:
        printf(",profile%" PRIu64,
               (uint64_t)(chunk->type & BTRFS_BLOCK_GROUP_PROFILE_MASK));
        break;
      }
    }
    printf("\t");
    ...
    if (chunk) {
      ...
      case BTRFS_BLOCK_GROUP_RAID10: {
        ...
        stripe_index = (stripe_nr % factor * chunk->sub_stripes);
        stripe_nr /= factor;
      ...
      case BTRFS_BLOCK_GROUP_RAID6: {
        ...
        if (chunk->type & BTRFS_BLOCK_GROUP_RAID6)
          ...
          nr_parity_stripes = 1;
        nr_data_stripes = (chunk->num_stripes - nr_parity_stripes);
        stripe_index = stripe_nr % nr_data_stripes;
        stripe_nr /= nr_data_stripes;
        stripe_index = ((stripe_nr + stripe_index) % chunk->num_stripes);
        num_stripes = 1;
        break;
      }
      default:
      ...
      for (i = 0; i < num_stripes; i++) {
        if (i != 0)
          printf("\n\t\t\t\t\t\t\t");
        printf("%" PRIu64 "\t%" PRIu64, chunk->stripes[stripe_index].devid,
